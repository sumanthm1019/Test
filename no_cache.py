...
class FileNode:
    ...
    def put(self,key,value):
        key = self.path+"&&"+key
	cursor = store.find({self.newkey(key) : {"$exists" : True}})
	for each in cursor:
		x = each['_id']
        store.update_one({'_id' : x}, {"$set" : {self.newkey(key) : (pickle.dumps(value))}})
	...
	cursor = store.find({self.newkey(key) : {"$exists" : True}})
	for each in cursor:
			x = pickle.loads(each[self.newkey(key)])
...
if __name__ == "__main__":
  if len(argv) != 4:
    print 'usage: %s <mountpoint> <mongodb url> <mongodb database name>' % argv[0]
    exit(1)
